...
title: |
  ...
  | Community Taxonomic Information
author: Zach S.L. Foster
output:
...
**Abstract:** The collection of environmental samples (soil, water, animal gut contents, etc.), and sequencing the contaned DNA, has resulted in large data sets containing community taxonomic information.
However, the hierarchical nature of taxonomic information makes manipulating these data sets difficult and many R packages made for this have similar, yet unique, data structures and functions.
We are working on developing the rOpenSci package `taxa`, in collaboration with Scott Chamberlain (rOpenSci), to provide a set of all-purpose classes and associated manipulation functions for taxonomic data, modeled after the `dplyr` data-manipulation philosophy.
...
An award to this project would help facilitate the continued development of standards for community taxonomic information in R, development of the `metacoder` as an example of an R package that works with these standards to visualize this data, and to disseminate these new tools through presentation at a useR! conference and online.

**Background:** Large data sets containing community taxonomic information are becoming increasingly common, but the hierarchical nature of taxonomic information makes manipulating these data sets difficult and many R packages made for this have similar, yet unique, data structures and functions.
...
**Proposed activities:** Although the `taxa` package is already useful, to make it a solid foundation for the community to build on, we want to make it faster, more robust, and work to integrate it with select packages that will speed adoption.
...
**Conclusion:** Encouraging adoption of this framework by both users and developers is key to the success of this project.
...
1. Port slowest parts of the code to C++ to improve speed
1. Add ability to use multiple cores to improve speed
1. Add exhaustive unit tests (test coverage of > 95%)
...
Niklaus Grünwald, Research Plant Pathologist, Horticultural Crops Research Laboratory, USDA-ARS, Corvallis, Oregon, United States of America 

## High level budget


```{r, echo = FALSE}
travel <- data.frame(matrix(ncol=4, nrow = 5))
names(travel) <- c('Item', 'Cost', 'Units', 'Total')
travel[1,1] <- "Airtavel: Portland, OR, USA to Brisbane, AU"
travel[1,2] <- 1400
travel[1,3] <- 1
travel[1,4] <- 1400
travel[2,1] <- "Lodging"
...
travel[3,3] <- "4 days"
travel[3,4] <- 98 * 4
travel[4,1] <- "Conference registration"
travel[4,2] <- 125
travel[4,3] <- 1
travel[4,4] <- 125
travel[5,1] <- "Grand total"
travel[5,4] <- sum(travel[-5,4])

options(knitr.kable.NA = '') 
knitr::kable(travel, align = c('l','c','c','r'), format.args = list(decimal.mark = ".", big.mark = ","), caption="Travel budget for 2018 useR! conference")
```
