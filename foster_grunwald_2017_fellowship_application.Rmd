...
**Abstract:**
The collection of environmental samples (soil, water, animal gut contents, etc.), and sequencing the contaned DNA, has resulted in large data sets containing community taxonomic information.
...
1. For each major file format used in metabarcoding research, add a function to parse the file’s contents into the classes provided by `taxa` and a corresponding writer to recreate the file format. We read from and written to the same format, the process should be lossless.
1. Add automatic overlap avoidance for labels
1. Add support for plotting categorical information
1. Add ability to plot multiple colors per node/edge automatically when multiple values per taxon are supplied. Nodes would be pie charts and edges would be stacked bar charts.
1. Add ability to automatically query, download and plot silhouettes representing `taxa` from the pylopics database in place of nodes
